...
(function () {
    ...
    class EventsController {
        ...
        startBackgroundService() {
            ...
            console.log("🤖 [EventsController_V5] Starting Background Automation Service...");
            this.state.bgInitialized = true;

            // 1. Data Listeners (Essential for Automation)
            this.unsubscribeEvents = window.db.collection('americanas').orderBy('date', 'asc')
                .onSnapshot(snap => {
                    this.state.americanas = snap.docs.map(d => ({ id: d.id, ...d.data() }));
                    this.onDataUpdate();
                });

            this.unsubscribeEntrenos = window.db.collection('entrenos').orderBy('date', 'asc')
                .onSnapshot(snap => {
                    this.state.entrenos = snap.docs.map(d => ({ id: d.id, ...d.data() }));
                    this.onDataUpdate();
                });

            // 2. Automation Loop (Run immediately + Interval)
            setInterval(() => this.checkAutoStartEvents(), 30000);
        }

        onDataUpdate() {
            // Always run automation checks on new data
            this.checkAutoStartEvents();

            // If View is active, re-render
            if (this.state.viewInitialized && !this.state.loading) {
                this.render();
            } else if (this.state.americanas && this.state.entrenos) {
                this.state.loading = false;
                if (this.state.viewInitialized) this.render();
            }
        }

        // Logic split: init() now specific for View/Rendering
        init() {
            ...
            console.log("🎟️ [EventsController_V5] UI Initialized.");

            // grab user from global store (auth) logic if needed
        ...
        hasEventStarted(dateStr, timeStr) {
            if (!dateStr || !timeStr) {
                console.warn("⚠️ hasEventStarted: Missing date or time", { dateStr, timeStr });
                return false;
            }
            try {
                let eventDate;
                ...
                if (dateStr.includes('/')) {
                    ...
                    eventDate = new Date(`${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}T${timeStr}:00`);
                } else {
                    eventDate = new Date(`${dateStr}T${timeStr}:00`);
                }

                const now = new Date();
                const hasStarted = now >= eventDate;

                console.log("🕐 hasEventStarted check:", {
                    dateStr,
                    timeStr,
        ...
        getEventTimes(dateStr, timeStr) {
            try {
                ...
                const start = new Date(`${dateBase}T${startStr}:00`);
                let end;
                if (endStr) {
                    end = new Date(`${dateBase}T${endStr}:00`);
                } else {
                    // Default 90 mins if no end time specified
        ...
        renderCard(evt, isLight = false) {
            ...
            let btnAction = `window.EventsController.joinEvent('${evt.id}', '${evt.type || 'americana'}')`;
            ...
            if (isFinished) {
                ...
                btnDims = 'width: auto; height: 45px; border-radius: 12px;';
                btnStyle = 'background: #FF2D55; color: white; padding: 0 18px; font-weight: 950; font-size:0.8rem; box-shadow: 0 4px 15px rgba(255, 45, 85, 0.4); animation: pulse 2s infinite; display:flex; align-items:center; gap:8px;';
                btnAction = `window.openLiveEvent('${evt.id}', '${evt.type || 'americana'}');`;

                // If not joined and not full, show secondary join button in card
                if (!isJoined && !isFull) {
                    // We'll add a secondary "+" button below
                }
            } else if (isJoined) {
                ...
                btnAction = `window.EventsController.leaveWaitlist('${evt.id}', '${evt.type || 'americana'}')`;
                btnDisabled = false;
            } else if (isFull) {
                ...
                btnAction = `window.EventsController.joinWaitlist('${evt.id}', '${evt.type || 'americana'}')`;
                btnDisabled = false;
            }
            ...
            return `
                ...
                <div class="card-hybrid-c" onclick="window.openLiveEvent('${evt.id}', '${evt.type || 'americana'}')" 
                     style="
                        position: relative; 
                    ...
                    <div style="position: absolute; top: 125px; right: 20px; z-index: 20; display: flex; align-items: center;">
                        ...
                        ` : ''}
                        <button onclick="event.stopPropagation(); ${btnAction}" ${btnDisabled ? 'disabled' : ''}
