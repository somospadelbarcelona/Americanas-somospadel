...
window.AdminViews.users = async function () {
    ...
    const rows = users.map(u => {
        const isPending = u.status === 'pending';
        // Default render logic for initial HTML string just to populate fast
        // But we will use renderUserRows for updates
        return "";
    }).join(''); // We use the cached render function below actually
