# Skill: Equipo Mendez AI (Multi-Agente)
...
## Roles del Equipo
1. **Director (Mendez AI)**: El líder. Divide el problema, asigna roles y aprueba planes.
...
Antes de realizar cambios significativos, cada agente debe enviar un **Plan de Acción** al buzón de Mendez AI.
- El agente se mantiene en modo `READ_ONLY` o `PLANNING` hasta que Mendez AI responda con un mensaje de `APPROVED`.

### 2. Mensajería y Difusión (Broadcast)
- **Mensaje Directo**: Coordinación 1 a 1 entre especialistas.
- **Broadcast**: Mendez AI puede escribir en `broadcast.msg` para dar nuevas directrices a todo el equipo simultáneamente.
...
- Al completar una tarea, el agente debe liberar sus "locks" y notificar a Mendez AI.
```
...
def init_team():
    ...
    print("✓ Infraestructura 'Equipo Mendez AI' lista.")
...
## 4. Cómo usarlo
1. **Activa el Líder**: Pídele a Antigravity: *"Usa la habilidad Equipo Mendez AI para inicializar este proyecto"*.
2. **Reparte el trabajo**: **Mendez AI** dividirá el trabajo. Abre terminales nuevas para cada agente (Frontend, Marketer, etc.).
3. **Flujo de Planificación**: Los agentes envían sus planes a Mendez AI antes de empezar. Un equipo bien coordinado es imparable.
