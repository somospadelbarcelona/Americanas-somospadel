    ...
    class AmericanaService {
        ...
        async generateNextRound(eventId, currentRound, type = 'entreno') {
            try {
                ...
                // CHECK IF NEXT ROUND ALREADY EXISTS (Idempotency Check)
                ...
                if (!existing.empty) {
                    console.log(`⚠️ [AmericanaService] Round ${nextRound} already exists. Skipping generation.`);
                    return { success: false, message: 'La siguiente ronda (R' + nextRound + ') ya existe.' };
                }
